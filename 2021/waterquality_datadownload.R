...
#Updated 2/4/2021 by Emily Houlihan, GAMA Unit (Emily.Houlihan@Waterboards.ca.gov)
...
rm(wells_int)
lat_long <- loc %>% select(WELL_ID, LONGITUDE, LATITUDE)
allwells <- left_join(allwells, lat_long)
write.table(allwells, paste0("allwells", Sys.Date(), ".txt"), sep = "\t", row.names = F, col.names = T)

#allwells <- read.table("allwells2021-02-05.txt", stringsAsFactors = F, header = T)
