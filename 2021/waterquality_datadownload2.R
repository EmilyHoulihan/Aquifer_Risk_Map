...
GU_depths <- read.table("workingdrafts/ref data/well_depth_gwunits2021-05-25.txt", stringsAsFactors = F, header = T)
...
domwqdata <- read.table("workingdrafts/domwqdata_2021-06-24.txt", stringsAsFactors = F, header = T)
...
domwqdata_sm <- domwqdata %>% select(gm_well_id, gm_samp_collection_date, gm_chemical_vvl,
  ...
  left_join(., select(domwells, gm_well_id, MTRS)) #attach MTRS for easy filtering later
write.table(domwqdata_sm, paste0("domwqdata_small_", Sys.Date(), ".txt"), sep = "\t", row.names = F, col.names = T)
