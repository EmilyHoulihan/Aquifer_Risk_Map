...
cbg2 <- CBG %>% select(GEOID, usgsdw_sum, usgsdw_mea) %>% distinct()  
...
domwelldata <- welldata %>% 
  filter(WELL_ID %in% domwells$WELL_ID, 
    !outlier_status == "outlier", !CHEMICAL == "TCPR123_2", !is.na(MCLindex)) %>% 
  mutate(year = year(DATE))
rm(welldata)

#CR6 adjustment - change to 10 ug/L (original has it at 20 ccv)
domwelldata2 <- domwelldata %>% mutate(MCLindex = ifelse(CHEMICAL == "CR6", MCLindex*2, MCLindex))

#de-cluster - calculate long-term average by year, then by well
domwellavg <- domwelldata2 %>% 
...
domwellre_long <- domwelldata2 %>% filter(DATE >= ymd("2018-01-01")) %>%
  mutate(re_status = ifelse(MCLindex > 1, 'over',
                            ifelse(MCLindex > 0.8, 'close', 'under'))) %>% 
  group_by(CHEMICAL, WELL_ID, re_status) %>% count()
domwellre_avg <- domwelldata2 %>% filter(DATE >= ymd("2018-01-01"), MCLindex > 1) %>% 
...
wellinfo <- allwells %>% select(WELL_ID, LATITUDE, LONGITUDE, SOURCE)
...
#write.csv(pdrisk, "pointdata_risk_cr6adj.csv", row.names = F)
...
domwellavg <- left_join(domwellavg, select(wellsmtrs, WELL_ID, MTRS)) %>%
...
domwellre <- left_join(domwellre, select(wellsmtrs, WELL_ID, MTRS))
...
write.csv(sdrisk, "sectiondata_risk_cr6adj.csv", row.names = F)
...
write.csv(cdrisk_wq, "censusdata_risk_wq.csv", row.names = F)
...
write.csv(comprisk_write, "comprisk_demo_dwsscount_oswcr1970na2.csv", row.names = F)

#Extra - make layers for individual chemicals ####
sdata3[is.na(sdata3)] <- 0
